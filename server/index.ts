...
// Log dotenv loading result for debugging (only in development)
// In production, environment variables should be set directly by the hosting platform
if (result.error && process.env.NODE_ENV !== 'production') {
  console.warn('[dotenv] Could not load .env file (this is normal in production):', result.error.message);
  console.warn('[dotenv] In production, set environment variables directly in your hosting platform');
...
// Allow specific origin from environment variable, or allow all origins if not set
const corsOptions = {
  origin: process.env.CORS_ORIGIN 
    ? process.env.CORS_ORIGIN.split(',').map(origin => origin.trim())
    : '*', // Allow all origins if CORS_ORIGIN is not set (useful for public APIs)
  credentials: true,
